...
void update_hour(Layer *me, GContext* ctx) {
  ...
  graphics_fill_rect(ctx, GRect(38, 131 - height * 10, 30, height * 10), 0, GCornerNone);

  for (int i = 0; i < height-1; i++)
  {
    graphics_draw_line(ctx, GPoint(38, 121 - i*10), GPoint(67, 121 - i*10));
  }
}
...
void update_minute(Layer *me, GContext* ctx) {
  ...
  graphics_fill_rect(ctx, GRect(76, 131 - height, 30, height), 0, GCornerNone);

  for (int i = 10; i < height; i+=10)
  {
    graphics_draw_line(ctx, GPoint(76, 131 - i), GPoint(105, 131 - i));
  }
}
...
void handle_init(AppContextRef ctx) {
  ...
  text_layer_init(&dateText, GRect(0,135,144,30));
  ...
  text_layer_set_font(&dateText, fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_ROBOTO_22)));
